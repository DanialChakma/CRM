...
function menu_controller(){
    ...
    if (auth_session_data != null) {
        ...
        if ( (auth_data.user_role).toLowerCase() == 'account_admin' ) {
            $(".accounts_admin").show();
            $(".accounts_admin li").show();
        }else if( (auth_data.user_role).toLowerCase() == 'account_agent' ){
            $(".accounts_admin").show();
            $(".accounts_admin li.account_agent").show();
            $(".accounts_admin li:not(.account_agent)").hide();

        }else if( (auth_data.user_role).toLowerCase() == 'account_caller' ){
            $(".accounts_admin").show();
            $(".accounts_admin").find("li.account_caller").show();
            $(".accounts_admin").find("li:not(.account_caller)").hide();
        }
    }
}
    ...
    if (cms_auth != null) {
        if (field_name == 'index') {
            ...
            if ( (auth_data.user_role).toLowerCase() == 'account_admin' ) {
                ...
                setTimeout(menu_controller,500);
                return;
            }else if( (auth_data.user_role).toLowerCase() == 'account_agent' ){
		...
		setTimeout(menu_controller,500);
            ...
            if((auth_data.user_role).toLowerCase() == 'asde'){
                ...
                /*  table_initialize_agent_wise_projected_sales_report();
                report_agent_wise_projected_sales_report(); */
            }
            ...
            view_accounts_tasks();
        }else if( field_name == 'account_caller' ) {
            monthly_call_page();
	     ...
	     view_accounts_tasks();
          ...
          //  view_all_task
            displayContent("60", "#cmsData", "#contentListLayout", "ContentID");
            fetchDropDownOptionHtml('#agent_select', cms_url['get_accounts_agent'], {"agent_type":"account_agent"});
            doze_DateTimePicker("reassign_date");
            calenderDatePicker(1, "get_account_manager_tasks");
            get_account_manager_tasks();
        ...
        } else if( field_name == 'month_wise_connection_report' ){
            displayContent("66", "#cmsData", "#contentListLayout", "ContentID");
            table_initialize_month_wise_connection();
            get_dataset_month_wise_connection();
        }  else if( field_name == 'current_month_connection_report' ){
            displayContent("67", "#cmsData", "#contentListLayout", "ContentID");
            table_initialize_current_month_connection();
            get_dataset_current_month_connection();
        } else if( field_name == 'agent_wise_connection_report' ){
            displayContent("68", "#cmsData", "#contentListLayout", "ContentID");
            table_initialize_agent_wise_connection();
            get_dataset_agent_wise_connection();
        } else if (field_name == 'agent_wise_pipeline_report') {
            displayContent("53", "#cmsData", "#contentListLayout", "ContentID");
        ...
        } else if (field_name == 'sqform') {
            displayContent("56", "#cmsData", "#contentListLayout", "ContentID");
            workflowPluginLoad();
            $('#sq_form input[name="action"]').val('insert');
            fetchDropDownOption('#agent_id', cms_url['doze_crm_sales_representative_list'], '');
            $('.ratingpoint').attr('disabled', 'disabled');
            $('.ratingYN').on('change', function () {
                if ($(this).val() == 'yes') {
                    $(this).closest("tr").find(".ratingpoint").removeAttr('disabled');
                    $(this).closest("tr").find(".ratingpoint").addClass('yesit');
                } else {
                    $(this).closest("tr").find(".ratingpoint option[value=0]").prop('selected','selected');
                    $(this).closest("tr").find(".ratingpoint").attr('disabled', 'disabled');
                    $(this).closest("tr").find(".ratingpoint").removeClass('yesit');
                }
            });
            calenderDatePicker(0);
        } else if (field_name == 'sq_report') {
            displayContent("57", "#cmsData", "#contentListLayout", "ContentID");
            fetchDropDownOption('#salesRepresentativeSQ', cms_url['doze_crm_sales_representative_list'], '');
            calenderDatePicker(1, "search_sq_report");
            table_initialize_sq_report();
            report_sq_report();
            ...
            fetchDropDownOption('#lead_source', cms_url['get_lead_source_list'], '');
            calenderDatePicker(0);
            ...
            calenderDatePicker(0);
        }else if (field_name == 'pipeline_health_report') {
            displayContent("54", "#cmsData", "#contentListLayout", "ContentID");
            calenderDatePicker(1,"search_pipeline_health_report");
            table_initialize_pipeline_health_report();
            report_pipeline_health_report();
        }else if (field_name == 'agent_wise_projected_sales') {
            displayContent("55", "#cmsData", "#contentListLayout", "ContentID");
            calenderDatePicker(1,"search_agent_wise_projected_sales");
            table_initialize_agent_wise_projected_sales_report();
            report_agent_wise_projected_sales_report();
        }else if (field_name == 'lead_status') {
            displayContent("52", "#cmsData", "#contentListLayout", "ContentID");
            calenderDatePicker(1, "search_lead_status_report");
            fetchDropDownOption('#select_lead_source', cms_url['get_lead_source_list'], '');
            table_initialize_lead_status_report();
            report_lead_status_report();
        }
    }
