...
while ($dt = Sql_fetch_array($result)) {
    ...
    $data_array[$i][$j++] = intval(100*(intval($dt['connected'])/intval($dt['attempted']))).' %';
    $data_array[$i][$j++] = intval(100*(intval($dt['interested'])/intval($dt['connected']))).' %';
    $data_array[$i][$j++] = intval(100*(intval($dt['verbally_confirmed'])/intval($dt['interested']))).' %';
    $i++;
    $serial++;
}
