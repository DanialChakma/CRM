...
//    }(SELECT client_type FROM customer_conversion WHERE customer_conversion.`contact_id`=contacts.`id` LIMIT 1) AS \\'
        $qry = 'SELECT contact_type,do_area,first_name, last_name, address1, address2, phone1, email FROM contacts WHERE id=' . $id;
        $res = Sql_exec($cn, $qry);
        while ($dt = Sql_fetch_array(($res))) {
            $datainfo['client_type'] = $dt['contact_type'];
            $datainfo['area'] = $dt['do_area'];
        ...
        $param = 'do=' . $datainfo['area'] . '&name=' . $datainfo['name'] . '&collection=' . $datainfo['collection_address'] . '&connection=' . $datainfo['address'] . '&contact=' . $datainfo['contact_number'] . '&email=' . $datainfo['email'] . '&type=' . $datainfo['client_type'] . '&time=' . $datainfo['assignment_date'] . '&package=' . $datainfo['package'] . '&receipt=' . $datainfo['receipt_number'] . '&install=' . $datainfo['assignment_date'].  '&conversion_note=' . $datainfo['conversion_note'] . '';
...
//if(trim($ticket_number) !='' && ($count ==0 || trim($count) =='0') ){
        ...
        $message .= "<b>Date of Installation:  </b>" . $datainfo['assignment_date'] . "</br>";
    ...
    $to = array("Ashiq@ssd-tech.com" => "Ashiq", "Zaman@ssd-tech.com" => "Zaman");
