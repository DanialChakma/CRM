...
//var_dump($_REQUEST); exit;
    ...
    if (isset($_REQUEST)) {
        ...
        $doze_id = substr($doze_id, 4);
    }
    if (strlen($doze_id)<5) {
//        echo json_encode($data_array);
//        exit;
        $doze_id='test@test.com';
    }
   $time_start = microtime_float();
    echo $api_url = "http://localhost/subscriptionservices_test/services/subscriber/SubscriberService.php?appid=test&apppass=test&cmdid=SHOW_SUB_LIST&cmdparam=where+id='" . trim($doze_id) . "'";
    $response = file_get_contents_user_define($api_url);

     $time_end = microtime_float();
        $time = $time_end - $time_start;    echo "<br>";
        echo "SHOW_SUB_LIST API CAll: $time  ";    echo "<br>";
    ...
    $today = date("Y-m-d");
  $time_start = microtime_float();    echo "<br>";
    echo $api_url = "http://localhost/subscriptionservices_test/services/subscriber/transactionhistory.php?appid=test&apppass=test&cmdid=SHOW_TRANSACTION_HISTORY&cmdparam=" . $uid . "|2013-08-03|" . $today;
    echo "<br>";
   ...
   $time_end = microtime_float();
        $time = $time_end - $time_start;    echo "<br>";
        echo "SHOW_TRANSACTION_HISTORY API CAll: $time  ";    echo "<br>";
    $resultResponse = explode("\n", $response);
